...
/obj/item/weapon/implant/mind_protect/loyalty/process()
	...
	if(prob(1) && prob(25))//1/400
		switch(rand(1, 8))
			if(1)
				to_chat(imp_in, "<span class='notice'>\italic Вы [pick("уверены", "считаете", "убеждены")], что НаноТрейзен - лучшая корпорация во всей Вселенной!")
			if(2)
				to_chat(imp_in, "<span class='notice'>\italic Вы [pick("уверены", "считаете", "убеждены")], что Капитан - величайший человек, который когда-либо жил!")
			if(3)
				to_chat(imp_in, "<span class='notice'>\italic Вы готовы отдать свою жизнь во славу НаноТрейзен!")
			if(4)
				to_chat(imp_in, "<span class='notice'>\italic Вы уверены в том, что все действия НаноТрейзен приведут к всеобщему благу!")
			if(5)
				to_chat(imp_in, "<span class='notice'>\italic Вы уверены в необходимости существования Космического Закона.")
			if(6)
				to_chat(imp_in, "<span class='notice'>\italic Вы отторгнете любую возможность предать НаноТрейзен.")
			if(7)
				to_chat(imp_in, "<span class='notice'>\italic Вы считаете всех несогласных с политикой НаноТрейзен неполноценными.")
			if(8)
				to_chat(imp_in, "<span class='notice'>\italic Вы уверены в необходимости существования Космического Закона")
